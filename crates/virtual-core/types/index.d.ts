// Hand-maintained public type surface for the vendored fork (consumers read
// this via the exports "types" condition and skipLibCheck it; the TS source
// in ../src is only type-checked by this package's own tsconfig). Keep new
// public options/methods in sync with ../src/index.ts.
...
export interface VirtualizerOptions<TScrollElement extends Element | Window, TItemElement extends Element> {
    ...
    /**
     * How estimate→measured corrections for content above the viewport keep
     * visible rows stable. 'offset' (default) writes the delta to scrollTop
     * (upstream behavior); 'origin' absorbs it into a layout origin baked
     * into row positions — no programmatic scroll can race the user's
     * gesture — and settles the accumulated debt with one verified write at
     * a safe moment (idle / debt cap / approaching the top).
     */
    scrollAnchoring?: 'offset' | 'origin';
    /**
     * Extends the visible window by this many pixels toward the user's last
